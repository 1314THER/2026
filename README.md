...
本包是这一题的**完整交付目录**：论文（含源文件与可提交的 PDF）、全部源程序、
结果数据与交付清单都在里面。

* 论文与提交格式：见 [`交付说明.md`](交付说明.md)（提交清单 + 逐条格式自查）；
  可提交的 PDF 是 `论文/论文全文_交付版.pdf`，`论文/论文全文.pdf` 是未缩放原版。
* 代码怎么读：见 [`代码/代码结构.md`](代码/代码结构.md)（分层地图 + 逐文件说明）
  与 [`代码/代码导读.md`](代码/代码导读.md)（逐段讲解求解器）。
* AI 使用情况：见 [`AI 使用报告.md`](AI%20使用报告.md)（规范要求在支撑材料中提供）。
...
| 四 | 计入收缩的烘干模型 | **51.08 h = 2.128 天** | `输出/result4.xlsx` |

两个求解器写的是同一套模型：圆柱坐标一维径向的耦合传热传质方程组，
节点中心有限体积离散 + 后向 Euler 时间推进 + Picard 迭代；水分方程四个问题
统一取经典 Fick 形式（体积密度只以密度乘比热进入热方程）。问题一物性为常数
（两方程解耦），问题二起物性与扩散系数随含水率、温度耦合；问题四把半径的
收缩写进贴体坐标 $\xi=r/R(t)$，此时对流项与坐标变换项**精确抵消**，方程形式
不变，收缩只通过度量因子 $1/R^2$ 与表面边界条件里的 $R$ 进入模型。
...
├── 打包支撑材料.sh                生成 ≤20 MB 的支撑材料 ZIP（提交用）
├── 交付说明.md                    提交清单 + 逐条格式自查（页数/页边距/页码/身份信息）
├── AI 使用报告.md                 AI 使用范围与人工把关方式（规范要求在支撑材料中提供）
│
├── 论文/
│   ├── 论文全文.tex              论文源文件（单文件，含附录全部程序清单）
│   ├── 论文全文.pdf              编译原版（未缩放，供继续修改）
│   ├── 论文全文_交付版.pdf        提交用：A4 + 四边页边距 ≥2.5 cm，分页与正文页数不变
│   └── figs/                     论文正文引用的 9 张插图源文件（矢量 PDF）
│
├── A题-药材的烘干问题.md          题面全文转写（含 6 张表与附录公式）
├── A题-药材的烘干问题.pdf         题面原始 PDF
├── 数据核对报告.md                论文数字的逐项核对结果（365 项 + 第三种口径补算）
...
│   ├── make_delivery_pdf.py      论文 PDF → 交付版（等比缩放到 A4 + 2.5 cm 页边距）
│   ├── 代码结构.md               分层地图 + 逐文件说明 + 运行顺序
│   ├── 代码导读.md               逐段讲解两个求解器
...
```bash
./reproduce.sh --fast                                        # 先跑一次，生成 输出/*_data.npz 求解缓存
./.venv/bin/python 代码/verify_paper_numbers.py --json 输出/数据自检报告.json
```

> 注意：`verify_paper_numbers.py` 要读 `输出/result1_data.npz`、
> `result23_data.npz`、`result4_data.npz` 三份求解缓存，而打包时按"可重算的大文件"
> 排除了它们；所以**必须先跑一次求解**（上面第一行，或完整的 `./reproduce.sh`），
> 否则第一步就会报 `FileNotFoundError`。
...
逐项核对的结果、以及本轮发现的 12 处数据问题与建议改法，整理在
[`数据核对报告.md`](数据核对报告.md)：论文表 1 到表 6 的 234 个单元格逐格零差异；
表 14 里第三种口径（干基密度守恒）此前在论文中写作 51.39 h，实际补算为 48.39 h
（N=100），该口径对应的实现见 `代码/closure_rhod.py`。
...
* 重新编译论文需要 LaTeX（Tectonic 或 TeX Live），生成提交版还要 PyMuPDF
  （`pip install pymupdf`，`代码/make_delivery_pdf.py` 用它拼接 A4 版）；
  这两项只影响论文 PDF 的重建，与数据复现无关。支撑材料包里不含已编译的论文 PDF。
...
* `solve_p234.py` 与问题一的关键差别：变物性、耦合迭代、问题四的贴体坐标；
...
论文是单文件 XeLaTeX 源 `论文/论文全文.tex`（附录按 `../代码/xxx.py` 直接嵌入
程序清单，所以源文件必须和 `代码/` 放在同一层的项目里）。本机没有 TeX Live 时，
可用 Tectonic 编译：

```bash
TECTONIC_CACHE_DIR=/tmp/tectonic-cache tectonic -X compile 论文/论文全文.tex \
    --outdir 论文 --outfmt pdf

# 生成提交用版本（A4 + 四边页边距 ≥2.5 cm，分页与正文页数保持不变）
python3 代码/make_delivery_pdf.py 论文/论文全文.pdf 论文/论文全文_交付版.pdf
```

`make_delivery_pdf.py` 会打印缩放比例、页数、页面尺寸与逐页实测的最小页边距；
它需要 PyMuPDF（`pip install pymupdf`），只做等比缩放与居中，不改变文字与矢量图。

交付流程（两个文件）：

```bash
./setup.sh              # 建环境（别人复核时也要能跑）
./reproduce.sh --fast   # 复现四个 result 文件（约 8 分钟）
./打包支撑材料.sh        # 生成 支撑材料.zip（默认约 6 MB，≤20 MB）
# 然后：论文/论文全文_交付版.pdf 与 支撑材料.zip 都按校内编号重命名后提交
```

提交格式的逐条自查（页数、页边距、页码位置、身份信息、AI 声明、体积）见
[`交付说明.md`](交付说明.md)。
