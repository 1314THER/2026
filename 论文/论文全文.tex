...
守恒残差达机器精度；网格与时间步都已收敛到本文所采用的有效位数——按第 \ref{sec:errorbudget} 节的说明，温度结果只有 3 位有效数字，表 \ref{tab:p1-T}
按四位小数报出，其末位是 $\Delta t=1$\,s 的时间离散误差（约
$2\times10^{-3}\,^\circ$C，比附件 1 的测量噪声小一个量级）。
...
$\Delta\le5\%/10\%/20\%$ 分别要求 $r\ge0.906/0.823/0.683$，比几何判据更严
（半径以平方进入特征扩散时间）。题面两式按任一口径都远低于阈值；
附录 \ref{app:shrink} 的参数扫描还给出：固定几何时 $r$ 对烘干时长几乎无
影响（全变程 $0.24\%$），一旦计入收缩，影响被放大到 $+3\%\sim+150\%$。
...
\begin{table}[htbp]
  ...
  \begin{tabular}{llcc}
    ...
    几何 & 固定半径 $R_0$（本文问题二/三） & $57.32$\,h & --- \\
         & 由 $\rho=a+bC$ 反演 $R(C)$ 自洽闭合 & $30.94$\,h & $-46.0\%$ \\
         & 附件 2 实测 $R(t)$（问题四的几何） & $25.21$\,h & $-56.0\%$ \\
    \midrule
...
计入由经验式反演的收缩使烘干时长缩短 $46.0\%$，计入实测收缩缩短 $56.0\%$；
...
（$30.94$\,h）与附件 2 实测收缩（$25.21$\,h）相差 $23\%$——
...
\begin{enumerate}
  ...
  \item 附件 1 的测量噪声给边界条件带来约 $0.02\,^\circ$C 的不可消除
        ...
        此外，本文时间方向用后向 Euler 一阶格式，$\Delta t=1$\,s（问题一）
        与 $\Delta t=5$\,s（问题二至四）的时间离散误差分别为
        $\sim2\times10^{-3}\,^\circ$C 与 $\sim7\times10^{-3}\,^\circ$C
        （后者出现在升温最快的 $t=0.5$\,h 附近），因此各表中的温度末位小数
        不具物理意义，判读时按 3 位有效数字处理；
...
\begin{table}[htbp]
  ...
  \begin{tabular}{cccccc}
    ...
    斜率 $b$ & $r=b/a$ & $t_{\text{固定}}$/h & $t_{\text{收缩}}$/h
      & $\Delta$ & 说明 \\
    \midrule
    $0$   & $0.000$ & $57.30$ & $22.96$ & $+150\%$ & 极限强收缩 \\
    $64$  & $0.098$ & $57.31$ & $27.05$ & $+112\%$ & \\
    $96$  & $0.148$ & $57.32$ & $29.02$ & $+97\%$  & \\
    $128$ & $0.197$ & $57.32$ & $30.94$ & $+85\%$  & \textbf{附录 3} \\
    $192$ & $0.295$ & $57.34$ & $34.66$ & $+65\%$  & \\
    $320$ & $0.492$ & $57.36$ & $41.63$ & $+38\%$  & \\
    $420$ & $0.646$ & $57.39$ & $46.72$ & $+23\%$  & \\
    $500$ & $0.769$ & $57.40$ & $50.59$ & $+13\%$  & \\
    $560$ & $0.862$ & $57.41$ & $53.39$ & $+8\%$   & \\
    $610$ & $0.938$ & $57.42$ & $55.66$ & $+3\%$   & \\
    $650$ & $1.000$ & $57.43$ & $57.43$ & $0$      & 斜率$=$截距，无收缩 \\
    \midrule
    \multicolumn{6}{l}{时长口径的阈值：$\Delta\le5\%\Rightarrow r\ge0.906$；
      $\Delta\le10\%\Rightarrow r\ge0.823$；$\Delta\le20\%\Rightarrow r\ge0.683$} \\
    \bottomrule
  \end{tabular}
\end{table}
...
被放大两个数量级：$r$ 从 $1$ 减到 $0$，$\Delta$ 从 $0$ 单调增到 $150\%$；
附录 3 的 $r=0.197$ 对应 $\Delta=85\%$。其三，把 $\Delta(r)$ 反解即得时长
口径的阈值：$\Delta\le5\%/10\%/20\%$ 分别要求 $r\ge0.906/0.823/0.683$，
