# ---------------------------------------------------------------------------
# 来源声明：本文件取自开源 skill 仓库 Escap1ng/mathmodel-kit（Apache-2.0）中的
#   skills/mathmodel-figure/code/style/plot_style.py
# 本项目仅做两处本地适配：
#   (1) pick_cjk_font 的候选字体表加入 macOS 常见中文字体（Songti SC / Heiti SC 等）；
#   (2) 顶部补上本来源声明。
# 取用目的是让本文插图的视觉语言（配色/字号/轴线）与该 skill 统一，
# 与另一个 skill（scipilot-figure-skill）驱动出的默认外观区分开。
# 原规范条文见同目录 nature-standard.md 与 visualization-rules.md。
# ---------------------------------------------------------------------------
...
# 本地适配：把 macOS 自带中文字体排到候选表里（原表只有 Windows/Linux 字体）。
# 排序依据：既要覆盖简体汉字，又要带粗体字重（标题与面板标号要加粗）。
# Hiragino Sans GB 是简体中文黑体、含 W3/W6 两个字重，实测无缺字；
# Songti SC / Heiti SC 无粗体字重，故排在后面；纯日文的 Hiragino Sans
# 缺简体字形（如"扩"），不能用作首选项。
def pick_cjk_font(candidates=('Hiragino Sans GB', 'PingFang SC',
                             'Microsoft YaHei', 'SimHei', 'Heiti SC',
                             'Noto Sans CJK SC', 'Songti SC', 'STHeiti',
                             'STSong', 'Arial Unicode MS',
                             'WenQuanYi Micro Hei')):
...
def pick_font_stack(latin=('Arial', 'Helvetica', 'Liberation Sans')):
    """构造 font.family 多字体回退栈。

    本地适配（重要）：原实现把 Arial 等西文字体放在最前、中文字体垫底。
    实测在 matplotlib 3.11 上，只要**一个字符串里同时出现中文与行内公式
    （$...$）**，逐字形回退就会失效：非公式段仍按字体栈首位的字体取字形，
    于是 "导热 $k\\,\\partial_r T$" 这类标签里的中文全部渲染成方框
    （stderr 会打印 "Font 'default' does not have a glyph ..."，
    但这是直接写 stderr 的，warnings/logging 都抓不到）。

    解决办法是把覆盖面最广的字体放到首位：优先 Arial Unicode MS
    （拉丁字形与 Arial 同族，且自带 CJK），其次本机可用的中文黑体。
    这样中文、西文、希腊字母三类字形都能从首位字体取到，混排不再出方框。
    """
    stack: list[str] = []
    # 首位字体还要负责行内公式里的 U+2212（减号）：对数坐标的刻度标签是
    # matplotlib 用 mathtext 生成的，形如 $\mathdefault{10^{-14}}$，
    # 其中的负号取的是 font.family 首位字体的字形。Hiragino Sans GB /
    # Songti SC 都没有 U+2212，实测会让对数刻度变成 "10¤4"。
    # Arial Unicode MS 同时具备 拉丁 + CJK + 数学符号（含 U+2212），故放首位。
    if 'Arial Unicode MS' in _INSTALLED_FONTS:
        stack.append('Arial Unicode MS')
    stack.append(pick_cjk_font())
    stack += [n for n in latin if n in _INSTALLED_FONTS and n not in stack]
    for extra in ('DejaVu Sans',):
